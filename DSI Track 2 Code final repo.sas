...
PROC IMPORT out=covid
	datafile='/home/u45096072/DSI competition database - Original.csv'
	DBMS=CSV replace;
	GETNAMES=YES;
RUN;
...
proc reg data=covid2;
	...
	model log_incidence = log_ranking;
	model log_incidence = log_hospital1;
	model log_incidence = log_hospital2;
...
proc corr data=covid2 plots(maxpoints=100000000)=matrix(nvar=all);
	var log_incidence log_black log_education log_income unemployment log_older_adults;
run;

proc corr data=covid2 plots(maxpoints=100000000)=matrix(nvar=all);
	var log_incidence log_ranking log_hospital2 log_hospital3 food_insecurity;
run;

proc corr data=covid2 plots(maxpoints=100000000)=matrix(nvar=all);
	var log_older_adults life_exp;
run;

proc reg data=covid2;
	model log_incidence = log_black log_education log_income unemployment log_older_adults log_ranking
						  log_hospital2 log_hospital3 food_insecurity diabetes_prev hypertension_prev
						  land_area life_exp / vif r;
run; quit;
...
PROC STANDARD DATA=covid2 MEAN=0 STD=1 OUT=covid_standardized;
	VAR log_black log_education log_income unemployment log_ranking
		log_hospital2 log_hospital3 food_insecurity diabetes_prev hypertension_prev
		land_area life_exp;
RUN;
...
PROC GLMSELECT DATA=TRAINTEST PLOTS=ALL SEED=123;
	...
	MODEL log_incidence = log_black log_education log_income unemployment log_ranking
						  log_hospital2 log_hospital3 food_insecurity diabetes_prev hypertension_prev
						  land_area life_exp / SELECTION=LAR (CHOOSE=CV STOP=NONE) CVMETHOD=RANDOM(10);
RUN;
...
proc glmselect data=covid_standardized plots=all;
	model log_incidence = log_black log_education log_income unemployment log_ranking
						  log_hospital2 log_hospital3 food_insecurity diabetes_prev hypertension_prev
						  land_area life_exp /selection=lasso(stop=none choose=bic);
run; 

*The remaining variables after lasso regression;
proc reg data=covid2;
	model log_incidence = log_black log_income food_insecurity life_exp land_area / vif r;
run; quit;

*Examine non-siginificant variables' effects on the model;
*Remove log_income;
proc reg data=covid2;
	model log_incidence = log_black food_insecurity life_exp land_area / vif r;
run; quit;
*Remoeve land_area;
proc reg data=covid2;
...
*Potential confounders based on knowledge;
proc reg data=covid2;
	model log_incidence = food_insecurity;
...
*Final model;
proc reg data=covid3;
	model sqrt_fatality = log_older_adults log_food2;
